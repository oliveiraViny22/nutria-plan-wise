...
-- Fix get_user_plan function to use type instead of name for free plan fallback
...
BEGIN
  ...
  -- If no subscription found, return free plan defaults using type instead of name
  IF NOT FOUND THEN
    RETURN QUERY
    SELECT 
      p.id as plan_id,
